...
---
Levcoin is a project for creationg of the first Bulgarian crypto-currency. It is based on CryptoNote source code, considered etter than the code of "bitcoin". Some changes in Econimic model are done. At the start of mining it is a bit faster that with bitcoin. But after that it becomes considerably slower. This is needed so "endless" mining is possible, that will ensure the presence of miners forever. And this will guarantee the survival of the currency.
...
May be thi problem will be solved in future, but now the risk is not addressed.
With LevCoin no such problems. Even in 30 years there still be what to mine. Based on this point the Bulgarian cryptocurrency is among the best in the world.
